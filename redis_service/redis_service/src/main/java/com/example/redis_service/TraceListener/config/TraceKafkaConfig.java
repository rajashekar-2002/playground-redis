package com.example.redis_service.traceListener.config;
...
public class TraceKafkaConfig {
    ...
    // StringDeserializer.class);
    // props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "latest"); // don't replay
    // old messages on restart

    // // Prevent ghost consumers from accumulating
    // props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, 6000); // detect dead
    // consumer in 6s (was 45s)
    // props.put(ConsumerConfig.HEARTBEAT_INTERVAL_MS_CONFIG, 2000); // heartbeat
    // every 2s
    // props.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, 10000); // kick if no
    // poll in 10s
    ...
    // traceKafkaListenerContainerFactory() {
